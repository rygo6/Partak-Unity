...
//			_itemSettings = ComponentUtility.FindOnNamedGameObject<ItemSettings>();
		...
		public void UnselectSelectedItem()
		{
			if (_selectedItemIndex != -1)
			{
				...
				_selectedItem = null;
				ItemBarHorizontalGroupItemList[_selectedItemIndex].GetComponent<Transform>().Find("ItemBarItemHighlight").GetComponent<Image>().enabled = false;
		...
		public void ItemButtonClick(int itemArrayIndex)
		{
			if (_selectedItemIndex != itemArrayIndex)
			{
				...
				else
				{
					...
					ItemBarHorizontalGroupItemList[_selectedItemIndex].GetComponent<Transform>().Find("ItemBarItemHighlight").GetComponent<Image>().enabled = true;
				}
			}
			else
			{
		...
		{
			AssetBundle assetBundle = null; //ComponentUtility.FindOnNamedGameObject<BundleStore>().LoadBundle(bundlePath);
