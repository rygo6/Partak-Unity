...
namespace Partak
{
    public class CellGroupGrid
    {
        ...
        public CellGroupGrid(ParticleCellGrid particleCellGrid, int playerCount)
        {
            Dimension = particleCellGrid.Dimension;
            Grid = BuildCellGroupLayerFromParticleCellGrid(particleCellGrid, this, playerCount);
        }
        ...
        CellGroup[] BuildCellGroupLayerFromParticleCellGrid(
            ParticleCellGrid particleCellGrid,
            CellGroupGrid parentCellGroupGrid, 
            int playerCount)
        {
            ...
            {
                if (particleCellGridArray[i].InhabitedBy != 255)
                {
            ...
            for (int i = 0; i < cellGroupArray.Length; ++i)
            {
                if (cellGroupArray[i] != null)
                {
                    ...
                    {
                        Vector2Int coordinate = CellUtility.GridIndexToCoordinate(i, particleCellGrid.Dimension) +
                                                CellUtility.DirectionToXY(d);
        ...
        CellGroup[] BuildParentCellGroupLayer(CellGroupGrid baseCellGroupGrid, CellGroupGrid parentCellGroupGrid, int playerCount)
        {
...
//						childCellGroupArray[i].InStepArray = 2;
        ...
        void ConnectStraightNeighborCellGroup(
            CellGroup parentCellGroup,
            CellGroup baseCellGroup,
            int oppositeChildCellGroupIndex,
            ...
            int diagonalDirection)
        {
            ...
            if (neighborCellGroup != null)
            {
                if (neighborCellGroup.CellGroupGrid.ParentLevel == parentCellGroup.CellGroupGrid.ParentLevel)
                {
                    ...
                    //make it so that the child of the neighbor cell group always flow upstream
                    neighborCellGroup.ChildCellGroupArray[oppositeChildCellGroupIndex]
                        .DirectionalCellGroupArray[oppositeDirectionA] = parentCellGroup;
                    neighborCellGroup.ChildCellGroupArray[oppositeChildCellGroupIndex]
                        .DirectionalCellGroupArray[oppositeDirectionB] = parentCellGroup;
                    neighborCellGroup.ChildCellGroupArray[oppositeChildCellGroupIndex]
                        .DirectionalCellGroupArray[diagonalDirection] = parentCellGroup;
                }
        ...
        void ConnectDiagonalNeighborCellGroup(CellGroup parentCellGroup, CellGroup baseCellGroup,
            int oppositeChildCellGroupIndex, int neighborDirection, int oppositeDirection)
        {
            CellGroup neighborCellGroup = baseCellGroup.DirectionalCellGroupArray[neighborDirection];
            parentCellGroup.DirectionalCellGroupArray[neighborDirection] = neighborCellGroup;
            if (neighborCellGroup != null)
            {
                neighborCellGroup.DirectionalCellGroupArray[CellUtility.InvertDirection(neighborDirection)] =
                    parentCellGroup;
                if (neighborCellGroup.CellGroupGrid.ParentLevel == parentCellGroup.CellGroupGrid.ParentLevel)
                {
                    neighborCellGroup.ChildCellGroupArray[oppositeChildCellGroupIndex]
                        .DirectionalCellGroupArray[oppositeDirection] = parentCellGroup;
                }
            }
        }

        void ConnectChildCellGroups()
        {
        }
    }
}